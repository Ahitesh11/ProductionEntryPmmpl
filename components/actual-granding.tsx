...
export default function ActualGranding() {
  const [data, setData] = useState<GrandingRecord[]>([])
  const [filteredData, setFilteredData] = useState<GrandingRecord[]>([])
  ...
  const [selectedRecord, setSelectedRecord] = useState<GrandingRecord | null>(null)
  ...
  const fetchData = async () => {
    setLoading(true)
    try {
      const response = await fetch("/api/sheets/get-data?sheet=Actual Granding")
      const result = await response.json()
      if (result.success && result.data.length > 0) {
        const allData = result.data
        const metaCols = new Set(["sheetName", "sheetname", "Timestamp", "timestamp", "rowIndex", "Actual1", "actual1"])
        const pendingRecords = allData.filter((record: GrandingRecord) => {
          const actual1Value = record.Actual1 || record.actual1 || record["Actual1"]
          const isPending = !actual1Value || String(actual1Value).trim() === ""
          const hasRealData = Object.keys(record).some(
            (col) => !metaCols.has(col) && record[col] !== undefined && record[col] !== null && String(record[col]).trim() !== ""
          )
          return isPending && hasRealData
        })
        setData(allData)
        ...
        if (pendingRecords.length > 0) {
          const allColumns = Object.keys(pendingRecords[0])
            ...
            .filter((col) => pendingRecords.some((record: any) => record[col] && String(record[col]).trim() !== ""))
  ...
  const openTallyModal = (rowIndex: number, record: GrandingRecord) => {
  ...
  const displayColumns = Array.from(visibleColumns)
    ...
    return (
      <>
        ...
        <div className="page-loading">
          <div className="spinner-ring amber" />
          <p className="loading-text">Loading granding data...</p>
        </div>
      </>
    )
  }
      ...
      <div className="page-root">
        ...
        <div className="page-header">
          <div className="page-header-left">
            <div className="page-tag amber-tag">GRANDING</div>
            <h2 className="page-title">Actual Granding</h2>
            <p className="page-subtitle">
              {filteredData.length} pending • {data.length - filteredData.length} completed
            </p>
          </div>
          <div className="page-header-actions">
            <button onClick={fetchData} className="action-btn amber-btn">
              <RefreshCw size={14} />
              Refresh
            </button>
            <button onClick={() => setShowColumnSelector(!showColumnSelector)} className="action-btn ghost-btn">
              Columns
            </button>
          </div>
        </div>
        ...
        {showColumnSelector && (
          <div className="col-selector">
            <div className="col-selector-grid">
              {columns.map((col) => (
                <label key={col} className="col-checkbox">
                  <input
                    type="checkbox"
        ...
        <div className="info-banner amber-banner">
          <span className="info-dot amber-dot" />
          <span>
        ...
        {filteredData.length === 0 ? (
          <div className="empty-state">
            <div className="empty-icon amber-icon">
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
              </svg>
            </div>
            <p className="empty-title">All tally entries are completed!</p>
            <p className="empty-sub">No pending granding records found.</p>
          </div>
        ) : (
              ...
              <thead>
                <tr>
                  {displayColumns.map((col) => (
                    <th key={col} className="th amber-th">{col}</th>
                  ))}
                  <th className="th amber-th action-th">Action</th>
                </tr>
              </thead>
              <tbody>
                {filteredData
                  .filter((record) => {
                    // Exclude meta/system columns that exist on every row including blank ones
                    const skipCols = new Set(["sheetName", "sheetname", "Timestamp", "timestamp", "rowIndex"])
                    return displayColumns.some(
                      (col) =>
                        !skipCols.has(col) &&
                  ...
                  .map((record, rowIdx) => (
                    <tr key={rowIdx} className="tr">
                      {displayColumns.map((col) => (
                        <td key={`${rowIdx}-${col}`} className="td">
                          {String(record[col] || "")}
                        </td>
                      ))}
                      <td className="td action-td">
                        <button
                          onClick={() => openTallyModal(record.rowIndex, record)}
                          className="tally-btn"
                        >
                          Tally Done
                        </button>
            ...
            <div className="modal-box">
              <div className="modal-header">
                <div className="modal-header-left">
                  <div className="modal-tag amber-tag">CONFIRM</div>
              ...
              <div className="modal-body">
                ...
                <div className="timestamp-card amber-timestamp">
              ...
              <div className="modal-footer">
                ...
                <button onClick={handleTallyEntry} className="action-btn amber-btn" disabled={isSubmitting}>
                  {isSubmitting ? (
...
const styles = `
  ...
  .spinner-ring.amber { border-top-color: #f59e0b; animation: spin 0.9s linear infinite; }
  ...
  .amber-btn { background: #f59e0b; color: #000; border-color: #f59e0b; }
  .amber-btn:hover { background: #fbbf24; box-shadow: 0 0 16px rgba(245,158,11,0.4); }
  ...
  .col-selector { background: rgba(245,158,11,0.05); border: 1px solid rgba(245,158,11,0.2); border-radius: 10px; padding: 14px 16px; margin-bottom: 16px; }
  .col-selector-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(140px, 1fr)); gap: 8px; }
  .col-checkbox { display: flex; align-items: center; gap: 6px; cursor: pointer; font-size: 11px; color: rgba(255,255,255,0.55); transition: color 0.15s; }
  .col-checkbox:hover { color: #f59e0b; }
  .col-checkbox input { accent-color: #f59e0b; width: 12px; height: 12px; }
  ...
  .amber-banner { background: rgba(245,158,11,0.07); border-color: rgba(245,158,11,0.2); color: rgba(255,255,255,0.5); }
  .info-dot { width: 6px; height: 6px; border-radius: 50%; flex-shrink: 0; }
  .amber-dot { background: #f59e0b; box-shadow: 0 0 6px rgba(245,158,11,0.7); }
  ...
  .amber-th { background: #f59e0b; color: #000; border-bottom: 2px solid #d97706; }
  ...
  .amber-icon { background: rgba(245,158,11,0.15); color: #f59e0b; border: 1px solid rgba(245,158,11,0.3); }
  ...
  .amber-timestamp { background: rgba(245,158,11,0.08); border-color: rgba(245,158,11,0.25); }
  ...
  .timestamp-value { font-family: 'JetBrains Mono', monospace; font-size: 15px; font-weight: 500; color: #f59e0b; letter-spacing: 1px; }
  .timestamp-format { font-family: 'JetBrains Mono', monospace; font-size: 9px; color: rgba(245,158,11,0.5); margin-top: 4px; letter-spacing: 0.5px; }
